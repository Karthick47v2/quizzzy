...
def preprocess_splitted_text(text):
    ...
    encode = sum_tokenizer.encode_plus(
...
def gen_question(context, answer):
    ...
    question_encoded = q_model.generate(input_ids=input_tokens_ids,
                                        ...
                                        # single question's max token len-- just an arbitary no -- but its enough
                                        max_length=72,
                                        early_stopping=True)

    # decode summarized token and post process it before print
    output = q_tokenizer.decode(
...
def send_to_fb(request):
    ...
    # update db
    docRef.update({'isGenerated': True})
